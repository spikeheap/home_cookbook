---
date: 2026-06-16
name: Cheese calzone
cuisine: Italian
meal: [Main]
effort: weekend
tags: [pizza, bread, vegetarian]
status: untried
description: A folded, stuffed pizza filled with three cheeses and tomato sauce.
isBasedOn: https://bakingsteel.com/blogs/recipes/classic-cheese-calzone-recipe
author:
  "@type": Person
  name: Andris Lagsdin
suitableForDiet:
- http://schema.org/VegetarianDiet

prepTime: PT15M
cookTime: PT20M
recipeYield: 1 calzone

recipeIngredient:
- heading: Dough
  items:
  - item: "1 ball [pizza dough](pizza_dough.html)"
- heading: Filling
  items:
  - item: "55g [pizza tomato sauce](pizza_tomato_sauce.html)"
  - quantity: 170
    unit: g
    item: ricotta
  - quantity: 170
    unit: g
    item: mozzarella (low-moisture), grated
  - quantity: 55
    unit: g
    item: Parmesan, grated
  - quantity: 50
    unit: g
    item: cooked ham or salami
  - quantity: 150
    unit: g
    item: spinach, wilted and dried ahead of time
    optional: true
recipeInstructions:
- items:
  - Stretch the dough into a 30cm circle on a floured peel.
  - Spread the ricotta, mozzarella, Parmesan and tomato sauce over one half, leaving a border. Add any ham, salami or spinach if using.
  - Fold the dough over into a half-moon, pinch the edges closed and trim about 5mm off the seam with a pizza wheel to seal. Poke 2-3 steam vents in the top.
  - Bake in the pizza oven for about 5 minutes, rotating regularly, until golden brown. Cool for 5 minutes before slicing; serve with extra tomato sauce for dipping.
---
