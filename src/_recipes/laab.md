---
date: 2026-05-29
name: Laab moo
cuisine: Thai
meal: [Main]
effort: weeknight
tags: [pork]

author:
  "@type": Person
  name: Pailin Chongchitnant

isBasedOn: https://hot-thai-kitchen.com/laab-moo/

prepTime: PT15M
cookTime: PT15M
recipeYield: Serves 3-4
servings: 3

recipeIngredient:
- heading: Toasted rice powder (kao kua)
  items:
  - quantity: 2
    unit: tbsp
    item: uncooked sticky rice (or jasmine rice)
  - quantity: 2
    item: kaffir lime leaves
    optional: true
- heading: Laab
  items:
  - quantity: 500
    unit: g
    item: ground pork
  - quantity: 60
    unit: ml
    item: water or unsalted chicken stock
  - quantity: 3
    unit: tbsp
    item: fish sauce
  - quantity: 3
    unit: tbsp
    item: lime juice (about 2 limes)
  - quantity: 1
    unit: tsp
    item: roasted chilli flakes (prik bon), or to taste
  - quantity: 3
    item: shallots, thinly sliced (red onion in a pinch)
  - quantity: 4
    unit: stalks
    item: spring onions, thinly sliced
  - quantity: 1
    unit: handful
    item: mint leaves
  - quantity: 1
    unit: handful
    item: coriander leaves, roughly chopped
- heading: To serve
  items:
  - item: sticky rice
  - item: raw vegetables — cucumber, long beans, cabbage, wedges of white cabbage

recipeInstructions:
- items:
  - Toast the rice in a dry pan over medium-high heat, stirring constantly, until deep golden brown. If using kaffir lime leaves, toast them with the rice until crisp. Cool slightly, then grind to a coarse powder in a mortar and pestle or spice grinder.
  - Heat the water or stock in a pan over medium heat. Add the pork and cook, breaking it up with a spoon, until just done — there's no need to brown it. Drain off any excess liquid.
  - Take the pan off the heat. Add the fish sauce, lime juice, chilli flakes and toasted rice powder. Stir to combine.
  - Add the shallots, spring onions, mint and coriander. Toss gently. Taste — adjust with more fish sauce (salt), lime (sour) or chilli (heat) to your liking.
  - Serve with sticky rice and a plate of raw vegetables for scooping and crunch.
---
